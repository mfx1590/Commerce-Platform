...
test('PLP → PDP → cart → checkout → confirmation', async ({ page }) => {
  ...
  await test.step('detail', async () => {
    await expect(page).toHaveURL(/\/en-GB\/products\/classic-tee$/);
  ...
  await test.step('checkout', async () => {
    ...
    await expect(page).toHaveURL(/\/en-GB\/checkout\/payment$/);
  ...
  await test.step('confirmation', async () => {
    await expect(page).toHaveURL(/\/en-GB\/orders\/[0-9a-f-]+$/);
...
test('the same page in German is translated and prices are formatted for de-DE', async ({
  ...
  // en-GB renders €19.99; de-DE puts the symbol last and uses a comma.
  await expect(page.getByTestId('price-value').first()).toHaveText(/19,99/);
});
