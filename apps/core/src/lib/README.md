...
| `payment-seam.ts` (the `PaymentProvider` types + registry + `manual` provider, re-exported by `src/modules/checkout`; task 2.5), `fraud-seam.ts` (`FraudCheck` types + registry + `evaluateFraud`: no check = allow, a throwing check = review; #231), `audit.ts` | `writeAudit(tx, entry)` — one `audit_log` row in the caller's transaction (Phase 1 stub for the auth-sdk helper); `Actor`, `SYSTEM_ACTOR`                                                  |
