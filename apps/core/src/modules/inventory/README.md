...
| `consumeReservationsForShipment(tx, { organizationId, storeId, orderId, shipmentId, warehouseId?, items: [{ orderLineItemId, quantity }], actor })` | window 8 (#191's `consumeReservations` shape)                                              | by order line item; **idempotent per shipment** (a `sale` movement referencing the shipment already exists for the variant → skipped)                                                                                                                                                                                                                                                                                                                                                                                      |
| `releaseReservationsForShipment(tx, { organizationId, storeId, orderId, shipmentId, items, actor })`                                                | window 8 (#191's `releaseReservations` shape): a planned shipment cancelled before picking | reverses what the shipment consumed for the lines in `items`: `adjustment` movement referencing `shipment_release:<id>`, reservation re-opened. Per variant the target is `min(consumed, asked)` minus what earlier calls released under this shipment, spent across the variant's warehouse rows in canonical order (priority, then code) — the same call twice releases once, a larger call releases the difference, empty `items` = everything the shipment still holds; returns the units released now                 |
