...
window 1 owns (registry, catalog, cart); everything else stays on the Prism mocks (`pnpm mock`). Owner: window 1 (core);
...
## What is real (Phase 2 · task 2.1)

| Surface          | Real in `@platform/core`                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Proxied / elsewhere                                                                                                                                                                                                                                          |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Store API        | `GET /store`, `GET /store/categories`, `GET /store/products`, `GET /store/products/{handle}` (tenant from `X-Publishable-Key`, `?currency=` one of the store's currencies, default the store currency) · **cart (2.1):** `POST /store/carts`, `GET`/`PATCH /store/carts/{cartId}`, `POST /store/carts/{cartId}/line-items`, `PATCH`/`DELETE /store/carts/{cartId}/line-items/{lineItemId}` — rows in `cart`/`cart_line_item`, totals in minor units, metadata round-trip | still proxied (2.2): `GET /store/carts/{cartId}/shipping-options`, `POST …/payment-session`, `POST …/complete`, `GET /store/orders/{orderId}`, `/store/customers*` → `CORE_STORE_API_FALLBACK_URL` verbatim (Prism mock, `:4010`); without the flag → Medusa |
...
5. Store API routes window 1 owns (`src/http/store-routes.ts`, `mountStoreRoutes`): `GET /store`,
   ...
   the spec, 404 for a cart outside the store, 409 `out_of_stock` / `cart_completed`). Exactly the contract shapes;
   they answer here, ahead of Medusa's routes of the same paths and of its publishable-key gate.
...
src/
  ...
  http/store-routes.ts      Store API handlers (contract routes: catalogue reads + cart), mounted ahead of Medusa by mountCoreMiddleware
