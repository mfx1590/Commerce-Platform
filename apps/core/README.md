...
| Admin API routes | `/admin/me`, stores, domains, sales channels, api keys, warehouses, legal entities, categories, products, variants, **orders (2.3): `listOrders` (filters, `q`, sort/order), `getOrder`, `cancelOrder`; **inventory (2.4): `GET /admin/inventory/levels`, `POST /admin/inventory/movements`; **returns (2.5): `POST …/orders/{orderId}/returns`, `POST …/returns/{returnId}/receive`** (window 1; other modules' routers mount via `src/http/module-routers.ts`: merchandising, media, price lists, promotions — window 9; marketing — window 17; shipments — window 8; and the provider webhooks `POST /webhooks/stripe/:storeCode` — window 7 — and `POST /webhooks/easypost/:storeCode` — window 8 — outside `/admin`, raw body) + `/admin/users*`, `/admin/audit-log`, `/admin/finance/ping` (hq-rbac, window 2)       | every other `/admin/*` path → Medusa (clients use the Prism mock `:4011`)                                                                                                                                           |
