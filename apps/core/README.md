...
window 1 owns (registry, catalog, cart, checkout, orders, inventory, returns); everything else stays on the Prism mocks (`pnpm mock`). Owner: window 1 (core);
...
## What is real (Phase 2 · task 2.6 — Phase 2 core complete)

| Surface          | Real in `@platform/core`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Proxied / elsewhere                                                                                                                                                                                                 |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Store API        | `GET /store`, `GET /store/categories`, `GET /store/products`, `GET /store/products/{handle}` (tenant from `X-Publishable-Key`, `?currency=` one of the store's currencies, default the store currency) · **cart (2.1):** `POST /store/carts`, `GET`/`PATCH /store/carts/{cartId}`, `POST /store/carts/{cartId}/line-items`, `PATCH`/`DELETE /store/carts/{cartId}/line-items/{lineItemId}` — rows in `cart`/`cart_line_item`, totals in minor units, metadata round-trip · **checkout (2.2):** `GET /store/carts/{cartId}/shipping-options`, `POST …/payment-session` (`manual` provider; Stripe = window 7), `POST …/complete` (`Idempotency-Key`; one transaction: `"order"`, `order_line_item`, `payment`, `attribution`, `order.placed`), `GET /store/orders/{orderId}` (customer token or `?email=`; 200 or 404 only)               | still proxied: `/store/customers*` (window 13) → `CORE_STORE_API_FALLBACK_URL` verbatim (Prism mock, `:4010`); without the flag → Medusa. The storefront checkout runs end to end on the core without the fallback. |
| Jobs             | `abandoned-carts` (Medusa scheduled job, `MEDUSA_WORKER_MODE = shared \| worker`; cron `CORE_ABANDONED_CART_CRON`, threshold `CORE_ABANDONED_CART_AFTER_HOURS`) marks idle carts `abandoned` and emits `cart.abandoned`; also `pnpm --filter @platform/core exec tsx src/jobs/abandoned-carts.ts [--hours 6] [--now <iso>]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | window 9's `index-products.ts` is a CLI (see its README)                                                                                                                                                            |
...
| Admin API routes | `/admin/me`, stores, domains, sales channels, api keys, warehouses, legal entities, categories, products, variants, **orders (2.3): `listOrders` (filters, `q`, sort/order), `getOrder`, `cancelOrder`; **inventory (2.4): `GET /admin/inventory/levels`, `POST /admin/inventory/movements`; **returns (2.5): `POST …/orders/{orderId}/returns`, `POST …/returns/{returnId}/receive`** (window 1; other modules' routers mount via `src/http/module-routers.ts`: merchandising, media, price lists, promotions — window 9; marketing — window 17; shipments — window 8; refunds — window 7; and the provider webhooks `POST /webhooks/stripe/:storeCode` — window 7 — and `POST /webhooks/easypost/:storeCode` — window 8 — outside `/admin`, raw body) + `/admin/users*`, `/admin/audit-log`, `/admin/finance/ping` (hq-rbac, window 2) | every other `/admin/*` path → Medusa (clients use the Prism mock `:4011`)                                                                                                                                           |
...
5. Store API routes window 1 owns (`src/http/store-routes.ts`, `mountStoreRoutes`): `GET /store`,
   `GET /store/categories`, `GET /store/products`, `GET /store/products/{handle}` (prices in `?currency=`, one of
   the store's currencies, default the store default currency; 400 `validation_error` otherwise), and the cart
   operations `POST /store/carts`, `GET`/`PATCH /store/carts/{cartId}`, `POST /store/carts/{cartId}/line-items`,
   `PATCH`/`DELETE /store/carts/{cartId}/line-items/{lineItemId}` (`src/modules/cart`; JSON bodies validated against
   the spec, 404 for a cart outside the store, 409 `out_of_stock` / `cart_completed`), and the checkout operations
   `GET /store/carts/{cartId}/shipping-options`, `POST …/payment-session`, `POST …/complete` → 201 `Order`,
   `GET /store/orders/{orderId}` (`src/modules/checkout`). Exactly the contract shapes; they answer here, ahead of
   Medusa's routes of the same paths and of its publishable-key gate.
...
src/
  ...
  http/store-routes.ts      Store API handlers (contract routes: catalogue reads, cart, checkout, order read), mounted ahead of Medusa by mountCoreMiddleware
