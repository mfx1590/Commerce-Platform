...
window 1 owns (registry, catalog, cart, checkout, orders); everything else stays on the Prism mocks (`pnpm mock`). Owner: window 1 (core);
...
| Admin API routes | `/admin/me`, stores, domains, sales channels, api keys, warehouses, legal entities, categories, products, variants, **orders (2.3): `listOrders` (filters, `q`, sort/order), `getOrder`, `cancelOrder`** (window 1; other modules' routers mount via `src/http/module-routers.ts`) + `/admin/users*`, `/admin/audit-log`, `/admin/finance/ping` (hq-rbac, window 2)                                                                                                                                                                                                                                                                                                                                                                                                                                                        | every other `/admin/*` path → Medusa (clients use the Prism mock `:4011`)                                                                                                                                           |
