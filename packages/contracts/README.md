...
| `openapi/admin-api.yaml` | 0.4.1          | 100        | registry, catalog, pricing, orders, inventory, fulfillment, customers, roles, audit, marketing, search |

`CONTRACTS_VERSION` is `0.4.1` (tag contracts-v0.4.1). The marketing area (37 operations, 15 components) is specified in
...
with `buy_x_get_y` and `stackable` / `exclusive` (#189) and a spelled-out `ProductFeed` (#194). Every admin operation
