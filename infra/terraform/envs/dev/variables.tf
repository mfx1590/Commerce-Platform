...
variable "base_domain" {
  description = "Public DNS zone. Service hostnames are <service>.dev.<base_domain>."
  type        = string
}

variable "bucket_prefix" {
  description = "Globally unique S3 name prefix (organisation slug). Buckets are <prefix>-dev-media / -backups."
  type        = string
}
