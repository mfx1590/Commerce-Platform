...
Last updated: 2026-09-05 · Contracts: contracts-v0.1 · Last commit: task 1.7 (branch `core/phase1-t7`, PR open) · Status: 1.1–1.7 done (PRs #49, #50, #51, #52, #54, #57, next), task 1.8 next — after the stack merges, fold back onto `core/phase1`
...
## In progress
- **Waiting for the manager (2026-09-05):** stack #49 → #50 → #51 → #52 → #54 → #57 → #58 unmerged. When all are merged: `git checkout core/phase1 && git merge main` (fold-back), delete the task branches, then task 1.8 on a branch off `core/phase1` with PR base `main`.
- **Task 1.8 plan (owner-scoped 2026-09-05, comment on #8):** `scripts/bootstrap.ts` + `pnpm --filter @platform/core bootstrap` = idempotent fail-fast verifier: (1) `packages/db` migrations + seed present (organization `hq`, 3 stores, `schema_migrations` current); (2) every seeded store has an active `web` sales channel and a non-revoked publishable key; (3) each seeded plain key resolves through `resolveStoreContext` to its store with the right default currency; (4) Medusa schema `medusa` exists with `link_module_migrations` rows (else point to `db:medusa:migrate`). Exit 1 with a precise message otherwise. Also run once at server start (warn, do not abort, unless `CORE_BOOTSTRAP_STRICT=1`). Test on a seeded throwaway DB + a test that a revoked seed key fails the verifier. No Medusa-side writes.

## Next — Phase 1 (GitHub issues #8–#9 are authoritative)
- [ ] 1.8 (#8, re-scoped) `bootstrap` verifier only — see In progress plan; the HTTP criterion (24/200 with the seeded key) already holds since #57
...
- 2026-09-05 · #40 is accepted on main (`default` export condition on db/events/contracts): after folding back onto `core/phase1` + `git merge main`, replace the dynamic `import()` shims (`src/lib/db.ts` initDb, `src/outbox/with-events.ts` eventsModule, `scripts/db-medusa-migrate.ts`) with static imports.
