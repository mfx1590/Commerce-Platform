...
## Done
- [x] **1.8 Bootstrap verifier (issue #8, re-scoped) + ts-node (#60)** — 2026-09-06, on `core/phase1` after #61 (`e9b5970`) merged; sha: see PR (filled at the next commit). Read-only readiness checks + CLI + server-start hook; no Medusa-side rows. `medusa build` verified on this tree.
...
## In progress
- **Task 1.8 (2026-09-06) built on `core/phase1` after #61 merged:** `src/bootstrap/{verify,index,README}.ts`, `scripts/bootstrap.ts`, server-start check (`CORE_BOOTSTRAP_STRICT`), 3 tests, `ts-node` devDep + build-tolerant config (#60). PR open; then 1.9, then sort/order.
- **Fold-back done (2026-09-05):** `core/phase1` = main (#49 merged) + replayed 1.2–1.7 + this fold-back commit (static imports, `CORE_DEV_TOKENS` opt-in, `requirePermission(relation, objectFactory)`, #48, lockfile regenerated against main). ONE PR covers 1.2–1.7; the manager reviews 1.7 from it. After it merges: 1.8 (verifier), 1.9, then the sort/order follow-up (Admin API 0.2.0) — each as one PR from `core/phase1`, waiting for the previous merge.
