...
- 2026-09-05 · Dev tokens (`Bearer dev:<subject>`) require `CORE_DEV_TOKENS=1` — explicit opt-in, never a NODE_ENV opt-out (manager). `requirePermission(relation, objectFactory)` is the middleware signature from `packages/auth-sdk/CLAUDE.md`; `can(principal, relation, object)` + `assertPermission` back it; admin routes chain `permission(op)` → `body(op)` → handler.
