...
- [x] **#137 · 2.4** Price lists — commit `<sha set at commit>` (local; same push hold). Module `apps/core/src/modules/promotions/` created (placement per the manager: the pricing half the cart consumes; recorded in the module README). No contract change needed: `listPriceLists`/`createPriceList`/`upsertPrices` exist in contracts-v0.3, so `pricingRouter()` takes permissions and bodies from `loadSpec` — first spec-driven router of this window. `resolvePrices` (sale > override/group > default, priority, windows, groups, tiers, Queryable-friendly for the cart's tx). REQUEST #179 (window 1): catalog media functions, three router mounts, cart `resolvePrices` call site. Tests: pricing.test.ts (5). Not built (not in the contract): list update/delete, CSV import; no price events exist (price changes reach the index at full reindex — documented).

## In progress
- (nothing — 2.5 starts next; on the manager's #166 merge confirmation: `git fetch` + merge origin branch if the manager updated it, `pnpm install`, module tests, then push and open TWO PRs is wrong — one branch one PR per task: push, open the 2.3 PR first ("search 2.3: Cloudinary media pipeline for product media (#136)"); the 2.4 commits ride the branch and become the 2.4 PR after the 2.3 PR merges — ask the manager whether to open both at once since GitHub allows only one open PR per branch: NO — one at a time, 2.3 then 2.4.)
