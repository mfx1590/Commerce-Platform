...
## In progress
- **#135 · 2.2 Merchandising rules API — PLAN, awaiting owner confirmation (2026-09-08).** 2.1 is PR #160 (awaits the Reviewer). Findings: Admin API 0.3.0 has no merchandising paths; db 0.2.0 has no rules table; Store API `sort` enum already has `relevance`.
  1. `CONTRACT CHANGE:` issue with exact diffs — Admin API: `GET/PUT /admin/stores/{storeId}/merchandising/rules` (list/replace) + `POST .../merchandising/rules/publish` (push to Algolia), `x-permission` `store_admin` write / `store_staff` read, schema `MerchandisingRule { id, scope: {type: category|query, category_id?|query?}, pins: [product_id…] (ordered), boosts: [{product_id, weight}], buries: [product_id…], enabled, starts_at?, ends_at? }`; db migration `0130_merchandising_rule` (`organization_id`, `store_id`, `scope_type`, `scope_key`, `rules jsonb`, `published_at`, RLS `store`). **Storage decision to confirm:** table (proposed, auditable, survives an index rebuild) vs Algolia rules as the only store (no migration, but lost on index drop and not queryable).
  2. Local mock: extend the module with an in-memory rules repository until the migration lands; route handlers in the search module (`src/modules/search/http.ts`) exposed through `index.ts` as `merchandisingRouter()`; mounting in `src/http/admin-routes.ts` is window 1's → REQUEST for the mount line (hq-rbac lesson).
  3. Rule → Algolia rule mapping (pure, unit-tested): pin = `consequence.promote`, bury = `consequence.hide`, boost = `consequence.params.optionalFilters` / `filterPromotes`; category scope = `condition.filters: category_id:<id>`, query scope = `condition.pattern` + `anchoring: is`. Validation: every product id belongs to the store (tenant client lookup), no duplicates across pins/buries, weights 1–100.
  4. `IndexClient` gains `saveRules(indexName, rules)` / `clearRules(indexName)` (fake + Algolia `/1/indexes/<name>/rules/batch`, `/rules/clear`); publish pushes all enabled rules of the store, `forwardToReplicas: false` (relevance only).
  5. Store API `sort=relevance` path: `searchProducts(client, store, query)` in the search module calling Algolia's query endpoint with `ruleContexts`; window 1 owns `store-routes.ts` → REQUEST for the call site; the ILIKE stub stays as fallback without credentials.
  Estimated ~35 tool calls; tests: mapping (pure), validation (DB), rules API through supertest with the dev-token verifier, fake client rules round trip.
