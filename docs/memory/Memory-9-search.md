...
Last updated: 2026-09-08 · Contracts: contracts-v0.3 (Store API 0.3.0, Admin API 0.3.0, events 0.2.0, db 0.2.0) · Branch: `search/phase2` · Status: 2.1 merged, 2.2 in merge queue (#166, behind #171), 2.3 + 2.4 built locally (push held until the manager confirms the #166 merge), 2.5 next
...
- [x] **#136 · 2.3** Cloudinary media pipeline — commit `097a9b5` (local; **do not push until the #166 merge is confirmed**). CONTRACT CHANGE #168 ACCEPTED (lands after the 2.3 PR merges, bundled into the same version bump as #162); REQUEST #169 (next/image loader, window 3). Media writes to `product_media`/`thumbnail_url` stay in the module for Phase 2 (manager decision); catalog public media functions requested in #179, switch when they exist. Tests: cloudinary.test.ts (7), media.test.ts (6).
- [x] **#137 · 2.4** Price lists — commit `46b4cf6` (local; same push hold). Module `apps/core/src/modules/promotions/` created (placement per the manager: the pricing half the cart consumes; recorded in the module README). No contract change needed: `listPriceLists`/`createPriceList`/`upsertPrices` exist in contracts-v0.3, so `pricingRouter()` takes permissions and bodies from `loadSpec` — first spec-driven router of this window. `resolvePrices` (sale > override/group > default, priority, windows, groups, tiers, Queryable-friendly for the cart's tx). REQUEST #179 (window 1): catalog media functions, three router mounts, cart `resolvePrices` call site. Tests: pricing.test.ts (5). Not built (not in the contract): list update/delete, CSV import; no price events exist (price changes reach the index at full reindex — documented).

## In progress
- **Ready to push, holding for the manager's word.** GitHub shows #166 MERGED and the manager's merge of main is folded into the local branch (typecheck + 44 module tests green after `pnpm install`). Local = origin + 6 commits (2.3 `097a9b5`, 2.4 `46b4cf6`, memory, merge). On confirmation: `git push origin search/phase2`, open the 2.3 PR ("search 2.3: Cloudinary media pipeline for product media (#136)"); one open PR per branch → the 2.4 PR follows once 2.3 merges.
- **#138 · 2.5 PLAN (awaiting the manager's go — budget rule, ~45 tool calls).** Contract findings: ops `listPromotions` (viewer; sort name/code/status/starts_at/created_at, paging) + `createPromotion` (store_admin) exist; `PromotionInput` has types `percentage|fixed_amount|free_shipping` only, `rules {min_subtotal_minor, product_ids, category_ids, customer_group_ids, sales_channel_ids, first_order_only}`, usage/per-customer limits, windows, status — but **no `buy_x_get_y` type, no stacking/exclusion fields, no update/disable op, no usage-increment path**; table `promotion` (0004) mirrors the schema (jsonb `rules`, `usage_count`); `getPromotionReport` (marketing tag) exists for #137's report — the data provider can serve window 17 through the module public API without owning the route. Steps:
  1. `CONTRACT CHANGE:` (exact YAML): add `buy_x_get_y` to the type enum + `rules.buy_quantity/get_quantity/get_discount_bp` (+ optional `get_product_ids`), add `stackable: boolean` (default false) + `exclusive: boolean` to Input/Promotion, add `PATCH /admin/stores/{storeId}/promotions/{promotionId}` (store_admin; status/limits/windows/rules). Table columns ride the jsonb `rules` + two new columns? NO new columns — keep `stackable`/`exclusive` inside `rules` jsonb to avoid a db CONTRACT CHANGE (frozen); flag the trade-off in the issue and let the manager choose (columns = queryable; jsonb = no migration).
  2. `promotions.ts` service: CRUD per contract (spec-driven router like pricing), code normalisation (upper, unique per store partial index → 409), validation (product/category/group/channel ids of the store; percentage value = basis points ≤ 10000; fixed_amount needs enabled currency).
  3. `engine.ts` (pure): `evaluatePromotions(cart, promotions, ctx)` → applicable set (conditions: min_subtotal, products/categories incl. subtree?, groups, first order, window, status, code match), stacking resolution (exclusive wins over everything and best-value among exclusives; stackables combine; non-stackable non-exclusive = best value), per-type discount (percentage on eligible lines, fixed_amount capped at subtotal, free_shipping flag, buy_x_get_y cheapest-eligible-free), **largest-remainder allocation across lines summing exactly to the discount** (integer rounding test), usage-limit check → contract error `promotion_usage_limit` (check error codes; likely `validation_error`/`conflict` — use `conflict`), expired code → 400/404 decision.
  4. `usage.ts`: `recordPromotionUse(tx, …)` increment with `usage_count < usage_limit` guard (row lock) + per-customer count from order metadata? No order table access — count via parameter from the cart/order caller; window 1 calls it at placement (REQUEST).
  5. `reports.ts`: `promotionReportData(client, storeId, from, to)` per `PromotionReport` schema, reading `promotion` + usage rows... uses/discount/revenue need order data — read `"order"` + `order_line_item`? Order tables are window 1's but read access is allowed (write-only restriction). Read `"order"` metadata/discount columns (check what core 2.2 wrote) — if orders don't carry promotion columns yet, provide uses/discount from usage tracking and zero revenue with a TODO + REQUEST to window 1. Decide at build time after inspecting the order schema.
  6. Router `promotionsRouter()` (spec ops + PATCH from the proposed YAML until the contract lands), tests (matrix per issue: stacking allowed/denied, exclusive wins, usage limit reached, expired code, allocation rounding, engine pure cases), README/CHANGELOG/memory, gates.

## Next — Phase 2 (GitHub issues; acceptance criteria there are authoritative)
- [ ] **#138 · 2.5** Promotions and coupon rule engine — `modules/promotions`: types percentage/fixed_amount/free_shipping (contract `PromotionInput` exists, tag `pricing`, ops `listPromotions`/… — check remaining ops around admin-api.yaml:762), buy-X-get-Y is NOT in the contract schema → check; conditions (min_subtotal, products/categories, customer group, first order), codes + usage limits, stacking/exclusion, evaluation for the cart via index.ts, `reports/promotions` data provider for window 17. Test matrix per the issue; discount allocation sums exactly (integer rounding test).
- [ ] **#166 reviewer nits (fold into the proposed/-cleanup follow-up PR)**: (1) `publishRules` calls Algolia inside the open transaction — push to Algolia first, then stamp `published_at` outside/after the tx; (2) the category-rule relevance assertion in merchandising.test.ts is conditional (`if (cat.ids.length > 0)`) and cannot fail — make it deterministic; (3) proposed/admin-api.merchandising.yaml header names `merchandising-schema.ts` which does not exist (`merchandising-types.ts`) — manager fixes when applying #162, fix ours in the cleanup.
- [ ] After #162 / #168 land: delete `proposed/`, switch `http.ts` + `media-http.ts` to `loadSpec('admin-api.yaml')` for permissions and bodies, switch merchandising.test.ts to the real migration (#100/#101 pattern follow-up PR) + the three nits above.
