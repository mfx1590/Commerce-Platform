...
## In progress
- **#120 · 2.2 Fetch layer, preview mode, revalidate-on-publish** — BUILT and green locally (2026-09-08: storefront typecheck, 184 tests incl. 4 new cms suites, root lint + format). Waiting for REQUEST #167 (`@platform/cms` dependency line in `apps/storefront-starter/package.json`, window 3's file) to land on main; then `git merge main && pnpm install`, drop the local uncommitted package.json line, open the PR. #164 (ownership for `src/app/api/cms/**`) was accepted on main a58c3da. Folded #163 nits: CHANGELOG count, `_validation` assertion (cms 0.1.1). Original plan for reference:
