...
Last updated: 2026-09-05 · Contracts: contracts-v0.1 · Last commit: 1d2dd52 (#43 follow-up; PR #38 with 1.1+1.2 MERGED 2026-09-05) · Status: Phase 1 in progress, parallel mode (worktree `../wt-auth`, branch `auth/phase1`)
...
- [x] 1.3 (#12) Tuple management — commit: pending (replay of parked 9c18c35; PR after #65 merges). auth-sdk `src/roles/service.ts` (`assignRole`/`revokeRole`: tuple → mirror+audit in one tx → tuple compensated on failure; idempotent; `ASSIGNABLE_RELATIONS` from model.fga; `listRoleAssignments`, `listStaffUsers`), `src/audit/write.ts` (`audit(tx, entry)`, org from tx context), `src/types.ts` (`StaffPrincipal`, `ApiError`, `forbidden()`), CLI `scripts/roles.ts`. hq-rbac: `http.ts` (`createHqRbac().handle(req)`: GET /admin/users, GET/POST /admin/users/{id}/roles, DELETE …/{assignmentId}; owner-on-organization:hq, 401/403/404/400/503 per contract), `test/roles.test.ts` (10 tests incl. 403 body for the 2-store admin on every route, audit rows, idempotency, tuple rollback, 503 fail-closed). `inviteUser` deferred to Phase 3.
