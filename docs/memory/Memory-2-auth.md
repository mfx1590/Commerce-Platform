...
## Next — Phase 1
- [ ] 1.2 (#11) `infra/openfga/model.fga` exactly per ADR 0002 + `tuples.seed.json` + `pnpm --filter @platform/auth-sdk fga:seed` (creates store/model/tuples on :8081, writes `OPENFGA_STORE_ID` to `.env`); model tests: store-admin → store_admin on brand-a/b, not finance on org; owner → viewer on every store. Relation names must equal `RELATIONS` in contracts.
...
- `pnpm --filter @platform/auth-sdk test` — static realm tests always run; live Keycloak tests run when `http://localhost:8180` answers.
