...
Last updated: 2026-09-05 · Contracts: contracts-v0.1 · Last commit: ae1d909 (task 1.3 replay, unpushed; #43 follow-up 1d2dd52 in PR #65) · Status: Phase 1 in progress, parallel mode (worktree `../wt-auth`, branch `auth/phase1`)
...
## Blocked / waiting
- REQUEST #48 (window 1): `apps/core/package.json` still lacks `@platform/auth-sdk` (checked 2026-09-06 after the Medusa merge); core's own typecheck/test go red once hq-rbac lands there. Module itself is green via auth-sdk's typecheck/test. Manager: do NOT touch that file from this window.
- REQUEST #48 (window 1 / manager): add `@platform/auth-sdk` + `@platform/db` to `apps/core/package.json`. Until then root `pnpm typecheck`/`pnpm test` are red for `@platform/core` (my module cannot resolve the packages inside core); the module itself is green through auth-sdk's typecheck/test. Not blocking my next tasks.
-  Keycloak `dev-mem` data loss was REQUEST #37 → fixed on main (bbb6259: `KC_DB: dev-file` + `keycloak-data` volume), issue closed.

## Gotchas learned
- **hq-rbac lives inside apps/core but core does not yet depend on my packages** (REQUEST #48) → the module is typechecked via its own `tsconfig.json` (paths → workspace src) run from auth-sdk's typecheck script, and its tests run from auth-sdk's vitest config (`resolve.alias` + an `include` that reaches `../../apps/core/...`). The module imports only `@platform/auth-sdk`, `@platform/db`(+`/testing`) and vitest — never `@openfga/sdk`/`pg` directly.
...
- **hq-rbac lives inside apps/core but core has no deps on my packages** → core's tsc/vitest pick the module up and fail (REQUEST #48). Testing/typechecking the module from auth-sdk works via vitest `resolve.alias` + the module's own `tsconfig.json` `paths`; the module must import nothing but `@platform/auth-sdk`, `@platform/db`(+ `/testing`) and `vitest` (no `@openfga/sdk`/`pg` directly — re-exported types from auth-sdk instead).
...
- `fga.read({user,relation,object})` is the cheap existence check before write/delete (OpenFGA rejects duplicate writes and missing deletes with 400).
